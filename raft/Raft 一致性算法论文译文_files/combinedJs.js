...
var ABTesting = {
	getABTestVersion : function(){
		// randVal can be 0 or 1 (Math.random returns in interval [0,1) )
...
// converts a string with format "HH:MM:SS" to seconds. Handles "HH:MM:SS" as well as "MM:SS" or "SS"
...
function appearedInView(element, minPercent) {
	var deferred = $.Deferred();

	element.viewable(function(event, percent) {
		if(percent >= minPercent) {
			deferred.resolve("isinview");
		}
	});
...
Newsletter.prototype = {
		saveSubscription : function(){
			...
			var handlerFunc = function(t) {
				var response = t;
				$('#'+nsltrObj.descriptionElementId).hide();
				if(response.code == 1){
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					'email':$('#' + this.newsletterElementId).val(),
					'emailH':$('#' + this.honeyPotId).val(),
					'fnt': $('#' + this.tokenId).val().split("").reverse().join("")
				},
				success: handlerFunc,
				error: errFunc
			});
		...
		validateEmail : function(i18nInvalidEmail){
			if($('#'+this.newsletterElementId).val().match(/^[a-zA-Z0-9_+&*-]+(?:\.[a-zA-Z0-9_+&*-]+)*@(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,7}$/gi)==null){
				$('#'+this.errorElementId).html(i18nInvalidEmail);
				$('#'+this.errorElementId).css("color","red");
				$('#'+this.errorElementId).show();
				$('#'+this.descriptionElementId).hide();
				return false;
			}
			return true;
		},
...
*/var CookieManager = {
	...
	createCookie : function(name,value,millis,domain) {
		if (millis) {
			var date = new Date();
...
var DynamicLinks = {
	updateRssLinks : function(fixedToken) {
		if(fixedToken) {
            ...
            var first = '/feed';
            rssUrl = InfoQConstants.countryCode + first;
...
var ContentTools = {
    /*
    Context: on the homepage grid page, the most popular widget needs to not exceed the height of the news section on the right.

    This function is used to adjust the hight of the widget by sequentially hiding tail elements in the list of most popular items
    until the hight is just right.
     */
	adjustPaneHeight : function(pane) {
        ...
        var podcastAdded = false;

        var $last = undefined;
        $(pane).children('h3').each(function () {
            var $this = $(this);
            $this.css('display', 'block');
            if($last != undefined && $last.hasClass('popular-podcast')) {
                podcastAdded = true;
            }
            if ($('#mostPopularSection').height() - 20 > heightToCover) { // each item has a bottom padding of 20px
                $this.css('display', 'none');
                if(!podcastAdded){
                    var podcastElement = $last.nextAll('.popular-podcast:first');
                    if(podcastElement.length) {
                        $last.css('display', 'none');
                        podcastElement.css('display', 'block');
                        while ($('#mostPopularSection').height() - 20 > heightToCover) {
                            $last = $last.prev();
                            $last.css('display', 'none');
                        }
                        podcastElement.addClass('last');
                    }
                    else {
                        $last.addClass('last');
                    }
	...
	showLoginWidget : function (anchor, origin, positionOnTop) {
        ...
        $('#login_email, #login_username, #login_emailVerification').blur(function(){
            var txt = $(this).val();
            $(this).val("");
            txt = txt.replace(/\n/g, ' ');
	...
	hookShowLogin: function (btnId, registerTrigger) {
		$(btnId).click(function() {
			UserActions_Login.showLoginWidget($(btnId),registerTrigger, false);
			return false;
		});
	},
	...
	hideLoginWidget : function () {
		...
		// update the top-right login button
		$("#login").removeClass('over');
                $('#loginWidgetOrigin').val(''); // added to avoid leaving it set - issues on modal P13N login
	},
	...
	showMessage : function(message) {
		$('[id^=loginPopupMessage]').css('color', '#48A657');
		$('[id^=loginPopupMessage]').html(message);
		$('[id^=loginPopupMessage]').show();
	},

    showMessageEmbedded : function(message) {
        $('#loginPopupMessageEmbedded').css('color', '#48A657');
        $('#loginPopupMessageEmbedded').html(message);
        $('#loginPopupMessageEmbedded').show();
    },

	showErrorMessage : function(message) {
		$('[id^=loginPopupMessage]').css('color', '#DD0B0B');
		$('[id^=loginPopupMessage]').html(message);
		$('[id^=loginPopupMessage]').show();
	},

    showErrorMessageEmbedded : function(message) {
        $('[id^=loginPopupMessageEmbedded]').css('color', '#DD0B0B');
        $('[id^=loginPopupMessageEmbedded]').html(message);
        $('[id^=loginPopupMessageEmbedded]').show();
    },

    getLoginWidgetOrigin : function() {
		return $('#loginWidgetOrigin').val();
	},
    prepareP13NLoginWidget: function(fromP13N, fromP13NId) {
        if($('#loginWidgetOrigin').val() === undefined || $('#loginWidgetOrigin').val()==="") {
			if (fromP13NId !== "") {
				$('#loginWidgetOrigin').val(fromP13N ? "P13N" : "");
			} else {
				$('#loginWidgetOrigin').val("P13NWelcome");
			}
		}
    },
    postLoginSuccessAction : function() {
		if(this.getLoginWidgetOrigin() == "mainLogin") {
			...
			UserActions_Profile.forceUpdateProfile(window.location.href, false);
		}
		else if(this.getLoginWidgetOrigin() == "rootCommentLogin") {
			this.hideLoginWidget();
            loggedIn = true;
            // just display popup here, force is done on post comment
            UserActions_Profile.forceUpdateProfile();
		}
		else if(this.getLoginWidgetOrigin() == "commentsLogin") {
			this.hideLoginWidget();
            loggedIn = true;
            // just display popup here, force is done on post comment
            UserActions_Profile.forceUpdateProfile(ReplyBoxHandler.showReplyBox);
		}
        ...
        else if(this.getLoginWidgetOrigin() == "minibookDownloadWithPhone") {
                this.hideLoginWidget();
                loggedIn = true;
                UserActions_Profile.forceUpdateProfile(window.location.href, false);
        }
            ...
            this.hideLoginWidget();
            loggedIn = true;
            UserActions_Profile.forceUpdateProfile(InterviewDownloads.loadMp3);
        }
        	...
        	UserActions_Profile.forceUpdateProfile($('#comeBackUrl').val(), false);
        }
            ...
            // just display popup here, force is done on post comment
            UserActions_Profile.forceUpdateProfile();
        } else if (this.getLoginWidgetOrigin() === 'P13N') {
            loggedIn = true;
            UserActions_Profile.forceUpdateProfile(window.location.href, false);
            //location.reload(); //removed since it seemed issues were caused by # in the link
        } else if (this.getLoginWidgetOrigin() === 'P13NWelcome') {
            loggedIn = true;
            UserActions_Profile.forceUpdateProfile(window.location.href, false);
            if(typeof destinationLink !== "undefined"){
...
//		    $('#'+stateDropdownId).append($('<option></option>'));
            ...
            // sort alphabetically
            zoneList.sort(function(valA, valB) {
                var stateA = valA.n.toUpperCase();
                var stateB = valB.n.toUpperCase();
	...
	validateKeepOrFieldNotName: function(keepFieldId,fieldId,validateDigit){
		if(!$('#'+keepFieldId).is(":checked")){
			return this.validateName(fieldId, validateDigit);
		}
		this.clearErrorOnField(fieldId);
		return true;
	},
	...
	checkEmail : function(email){
		$.ajax({
			...
			type: 'post',
			data: {'emailStr': email, 'cet':InfoQConstants.cet},
			async:false,
			success:function(data){
				UserActions_Profile.validateEmailCallback(data);
			},
		});
	},
	
	validateEmail : function(fieldId, invalidMessage, emailExistMessage) {
		var emailStr = $('#' + fieldId).val().trim();
		...
		if(!emailStr.match(/^[a-zA-Z0-9_+&*-]+(?:\.[a-zA-Z0-9_+&*-]+)*@(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,7}$/gi)){
		...
		if(emailStr.match(/@mailmetrash.com|@majou.org|@mailinator.com|@guerrillamail|@sharklasers.com|@jetable.org|mailexpire.com|@TempEmail.net|@spamgourmet.com|@ah.mintemail.com|@spamfree24.org|@deadaddress.com|@yopmail.com|@tempinbox.com|@mailcatch.com|@rtrtr.com|@tempomail.fr|@spamspot.com/gi)){
			$('#error_' + fieldId).html(invalidMessage);
			$('#error_' + fieldId).show();
			this.setErrorOnField(fieldId);
			this.emailHasErrors = true;
	...
	validateEmailCallback : function(data) {
		switch(data.code){
			...
			case "2":$('#error_' + UserActions_Profile.emailFieldId).html(UserActions_Profile.invalidEmailMessage);UserActions_Profile.emailHasErrors=true;break;
	...
	validateNotSingleChar : function(fieldId) {
		//for zh and ja always return true as this languages can heva a single char name
		if(InfoQConstants.language=='zh' || InfoQConstants.language=='ja'){
			return true;
		}
		var notSingleChar = false;
		if($('#' + fieldId).is("input")) {
			if($('#' + fieldId).val().length>1) {
				notSingleChar = true;
			}
		}		
		return notSingleChar;
	},
	
	validateNoDigit : function(fieldId) {
		var noDigit = true;
		if($('#' + fieldId).is("input")) {
			if(/\d/.test($('#' + fieldId).val())) {
				noDigit = false;
			}
		}
		return noDigit;
	...
	validateName : function(fieldId, validateDigit) {
		var valid = this.validateNotEmpty(fieldId) && this.validateNotSingleChar(fieldId);
		if(validateDigit){
			valid = this.validateNoDigit(fieldId) && valid;
		}		
		this.clearValidOnField(fieldId);
		this.clearErrorOnField(fieldId);
	...
	validateRegisterForm : function() {
		var errorFree = this.validateName('userprofile_firstName', true);
			errorFree = this.validateName('userprofile_lastName', true) && errorFree;
			...
			errorFree = this.validateName('userProfile_companyName', false) && errorFree;
	...
	validateAskLoginForm : function() {
		var errorFree = this.validateNotEmpty('askLogin_email');
		errorFree = this.validateNotEmpty('askLogin_password') && errorFree;
	...
	forceUpdateProfile: function(callback, force){
		...
		if((typeof loggedIn != "undefined") && loggedIn){
			if(InfoQConstants.isUserBlocked==true || InfoQConstants.isUserOutdated==true)
			{
				...
				forceUpdateColobox.colorbox({inline:true, opacity: 0.5, fixed:false, overlayClose:false, onClosed:this.forcePopupClose});
				forceUpdateColobox.click();

				// log the display popup action as metric on the server
				UserActions_Profile.doPostMetric(InfoQConstants.loggedUserEmail, InfoQConstants.language, popupAction);
			}
			else{
	...
	doPostMetric: function(email, language, action) {
		var jsonuser = [{"email": email,"language": language ,"action": action, "type":"user_stats"}];
		$.ajax({
			url: "/metrics/ajaxMetricsLogAction.action",
			contentType: "application/x-www-form-urlencoded; charset=utf-8",
			type: 'POST',
			data: {"json": JSON.stringify(jsonuser)}
		});
	},
	
	validateForceUpdateProfileForm: function(){
		
		var errorFree = this.validateKeepOrFieldNotName('keepCompany','company', false);
	...
	saveProfileUpdate: function(){
		if(this.validateForceUpdateProfileForm()==true){
			$.ajax({
				url: InfoQConstants.countryCode+"/updateuser.action",
				contentType: "application/x-www-form-urlencoded; charset=utf-8",
				type: 'POST',
	...
	forcePopupClose: function(){
		// for header login we need to execute the callback even if the popup is not submitted
		if(UserActions_Profile.forceUpdate == false){
			UserActions_Profile.forceUpdateExecuteCallback();
		}
	},
	
	forceUpdateExecuteCallback: function(){
		// we have cases where there is no callback (like comments post)
		if(typeof forceUpdateCallback !=='undefined'){
			if(typeof forceUpdateCallback ==="function"){
				forceUpdateCallback();
				// we can not close the colobox popup here as other popups might have issues(like in vcr questions)
			}else{
				// if it is not a function it is a URL to forward to
				window.location.href=forceUpdateCallback;
				// in case the content type makes the browser download the asset close the popup so it does not remain opened
				$.colorbox.close();
			}			
		}else{
			// close the popup, since the popup is closed in the callbacks usually here we need to close it ourselves as we have no callback
			$.colorbox.close();
		}
	},
	
	processResultForForceProfileUpdate: function(data){
		// update successful reset blocked and outdated flags
...
UserActions_Minibooks = {
		donwloadBook : function(){
			// we might have the force popup opened here close it (some urls like epub are interpreted as download and the current window does ot change)
			$.colorbox.close();
			window.location.href=location.protocol+'//'+location.hostname+(location.port ? ':'+location.port: '')+$('#dldLinkFinal').val();
			if (InfoQConstants.language=='zh'){
				window.location.href = $('#dldLinkFinal').val();
			}
		}
	};
...
UserActions_RestrictedPresentations = {
		...
		shareTokenValid : function(tokenValue) {
			if(tokenValue){
				re = /^[a-zA-Z0-9_+&*-]+(?:\.[a-zA-Z0-9_+&*-]+)*@(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,7}$/gi;
		...
		validateEmails : function(csvEmails) {
			if(!trim(csvEmails)==""){
				...
				for(var i=0;i<emails.length;i++){
					if(!emails[i].match(/^[a-zA-Z0-9_+&*-]+(?:\.[a-zA-Z0-9_+&*-]+)*@(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,7}$/gi)){
						this.hasErrors=true;
					}
				}
			}
...
var UserActions_ResetPass = {
		validateEmail : function(fieldId, errorFieldId, invalidMessage){
			var emailStr = $('#' + fieldId).val().trim();
			$('#' + errorFieldId).hide();			
			// check for a valid email address
			if(!emailStr.match(/^[a-zA-Z0-9_+&*-]+(?:\.[a-zA-Z0-9_+&*-]+)*@(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,7}$/gi)){
				$('#' + errorFieldId).html(invalidMessage);
				$('#' + errorFieldId).css("color","red");
				$('#' + errorFieldId).show();
				return false;
			}
			// check for spam eater email accounts
		...
		checkInputStatus : function(fieldId, errorFieldId, invalidMessage, submitButtonId){
			$('#' + errorFieldId).hide();
			if(this.validateEmail(fieldId, errorFieldId, invalidMessage)){
				$('#' + submitButtonId).addClass( "active" );
				$('#' + submitButtonId).removeAttr('disabled');
				return true;
			}else{
				$('#' + submitButtonId).removeClass( "active" );
...
var UserActions_EditorForm = {
		
		handleFileSelect : function(evt,  fieldId, sizeMessage, contentTypeMessage, dimensionMessage) {
			if (window.FileReader) {
				var files = evt.target.files; // FileList object
				if (files && files[0]) {
					UserActions_EditorForm.readImg(files[0], fieldId, sizeMessage, contentTypeMessage, dimensionMessage);
				}
			}else{
				alert("HTML5 Not Supported! Image upload is not available!");
			}

		},
		
		readImg : function(file, fieldId, sizeMessage, contentTypeMessage, dimensionMessage){
			if(UserActions_EditorForm.validateSize(file)){
				UserActions_Profile.clearErrorOnField(fieldId);
				$('#error_userprofile_picture').hide();
				if(UserActions_EditorForm.validateContentType(file)){
					UserActions_Profile.clearErrorOnField(fieldId);
					$('#error_userprofile_picture').hide();
			        ...
			        reader.onload = function() {// file is loaded
			            ...
			            image.onload = function() {
			                ...
			                if(w<900 && h<900){
			                	...
			                	UserActions_EditorForm.setErrorOnField(fieldId, dimensionMessage);
			                }
			            }
			        }
			        reader.readAsDataURL(file);//call readAsDataURL() to start the read operation in the background.
				}else{
					UserActions_EditorForm.setErrorOnField(fieldId, contentTypeMessage);
				}
			} else{
				UserActions_EditorForm.setErrorOnField(fieldId, sizeMessage);
			}
		},
		
		validateContentType : function(file){
		 ...
		 * 
		 * Set setErrorOnField and Clear the file input
		 * */
		setErrorOnField : function(fieldId, message){
			UserActions_Profile.setErrorOnField(fieldId);
			$('#error_userprofile_picture').html(message);
			$('#error_userprofile_picture').show();
			var control = $('#' + fieldId);
...
var UserActions_Notifications = {

	populateAgoTimes : function (selector) {
		'use strict';
		var that = this;
		$(selector).each(function () {
	...
	populateAgoTime : function (flatNotificationDiv) {
		'use strict';
		var commentId,
			agoTime = this.getRelativeTime($('input[name=tmstmp]', flatNotificationDiv).attr('value'));
		// populate the notification relative time
		$('span', flatNotificationDiv).text(agoTime);
	},
	...
	getRelativeTime : function(timeInMilliseconds) {
		...
		if(momentNow.diff(momentDate,"days")>=100){
	...
	markAsRead: function(ids){
		$.ajax({
			url: InfoQConstants.countryCode+"/notifications/markAsRead.action",
			contentType: "application/x-www-form-urlencoded; charset=utf-8",
			type: 'POST',
			data:{"is": ids}  ,
			success: this.processResultForMarkAsRead
		});
	},

	markAsDisplayed: function(){
		$.ajax({
			url: InfoQConstants.countryCode+"/notifications/markAsDisplay.action",
			contentType: "application/x-www-form-urlencoded; charset=utf-8",
			type: 'POST',
			success: this.processResultForMarkAsDisplay
		});
	},

	processResultForMarkAsRead : function(data){
		// json: {"code":"0=success/3=error", "message":"id_notification/all"}
		if(data.code == 0){
			//success, see if mark all was clicked or mark a single notification
			var notifCount = $(".notificationWidget__circle .circle-inner").html();
			if(data.message =="all") {
				$(".notificationWidget__item").removeClass("notificationWidget__notread");
				// hide all icons from notifications for mark as read
				$(".notificationWidget__icon-container").hide();

				// update notifications page in case this comes from notifications page
				$(".notification_page__item").removeClass("notification_page__notread");
			}else{
				// we have an id here
				$("#notif_"+data.message).removeClass("notificationWidget__notread");
				// hide all icons from notifications for mark as read
				$("#notif_"+data.message+" .notificationWidget__icon-container").hide();

				// update the notifications page in case the action comes from this page
				$("#notif_page_"+data.message).removeClass("notification_page__notread");
				// hide all icons from notifications for mark as read
				$("#notif_page_"+data.message+" .notification_page__icon-right").hide();
			}
		}
		else {
			// we have an error, how to display it?
		}
		Tracker.doTrackP13nNotificationEvents("mark_all_read", "desktop");
	},

	processResultForMarkAsDisplay : function(data){
            if(!loggedIn) {
                $(".notificationWidget__circle .circle-inner").text("");
                $(".notificationWidget__circle").hide();
            } else {
		// json: {"code":"0=success/3=error"}
		if(data.code == 0){
	...
	validateNotificationsSettingsForm : function(){
		return this.validateNotificationsLanguages();
	},

	validateNotificationsLanguages : function(){
		//reset error class and fields
		$("#notificationsLanguages span").removeClass("color--red");
		$("#notificationsLanguages span.check-error").hide();
		var oneChecked=false;
		$("#notificationsLanguages :checkbox").each(function () {
			if (this.checked){
				oneChecked=true;
			}
		});
		if(!oneChecked){
			$("#notificationsLanguages span").addClass("color--red");
			$("#notificationsLanguages span.check-error").css("display","block");
		}
		return oneChecked;
	}
};
var ContentRating = ( function () {
    ...
    var fieldsetIdRelevance = "relevance_fieldset";
    var fieldsetIdStyle = "style_fieldset";
    var content = {};
    var messages = {};

    function postData(){
        if(validateForm()){
            var relevance = $('#' + fieldsetIdRelevance+" :radio:checked").val();
            var style = $('#' + fieldsetIdStyle+" :radio:checked").val();
            var dataToPost = content;
            dataToPost.style = style;
            dataToPost.relevance = relevance;
            if($('#' + 'editor_input').val()){
            	dataToPost.editorInput = $('#' + 'editor_input').val();
            }
            if($('#' + 'chiefEditor_input').val()){
            	dataToPost.chiefEditorInput = $('#' + 'chiefEditor_input').val();
            }
            var authorContacted = $('#'+'auth-checkbox').is(":checked");
            dataToPost.authorContacted = authorContacted;
            jQuery.ajax({
                type: "POST",
                url: '/editor/contentRating.action',
                data: dataToPost,
                timeout: 3000,/*3 seconds*/
                success: function(data, textStatus, jqXHR){
                   if(data && data.ok){
                	   serverCallFeedback(true);
                   }else{
                	   serverCallFeedback(false);
                   }
                },
                error: function(){
                    serverCallFeedback(false);
    ...
    function serverCallFeedback(isSuccessful){
        var spanElem = $(".serverCallFeedback");
        if(isSuccessful){
            spanElem.html(messages.submitSuccess);
            spanElem.addClass('validated');
        }else{
            spanElem.html('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;');
            spanElem.addClass('error');
        }
    }
    
    function clearServerCallFeedback(){
        var spanElem = $(".serverCallFeedback");
        spanElem.html('');
        spanElem.removeClass('validated');
        spanElem.removeClass('error');
        spanElem.removeClass('invalid');
    }

    /**
     * Validates all the Controls
     * */
    function validateForm() {
        var relevanceValid = validateField(fieldsetIdRelevance);
        var styleValid = validateField(fieldsetIdStyle,relevanceValid);
        return relevanceValid && styleValid;

    }

    function validateField(fieldId, isPreviousFiledValid) {
        var filedValue = $('#' + fieldId + " :radio:checked").val();
        if (filedValue) {
            clearErrorOnField(fieldId, isPreviousFiledValid);
            return true;
        } else {
            setErrorOnFiled(fieldId);
            return false;
        }
    }

    function setErrorOnFiled(fieldId){
        $('#' + fieldId +" > legend").addClass('invalid');
        var spanElem = $(".serverCallFeedback");
        spanElem.html(messages.ratingRequired);
        spanElem.addClass('invalid');
    }

    function clearErrorOnField(fieldId, isPreviousFiledValid){
        $('#' + fieldId+" > legend").removeClass('invalid');
        if(isPreviousFiledValid===false){
            //Special UseCase:
            //Relevance is empty
            //but Style is filled in
            //ERR=> it doesn't show 'rating is required'
        }else{
            clearServerCallFeedback();
        }
    }

    function readContentItem(){
    	content.title =  $('#cr_item_title').val();
        content.author =  $('#cr_item_author').val();
        content.itemUrl =  $('#cr_item_url').val();
        content.lang =  $('#cr_item_lang').val();
        content.contentType = $('#cr_item_ctype').val();
        content.published = $('#cr_item_published_time').val();
        content.primaryTopic = $('#cr_item_primary_topic').val();
    }

    /**
     * Read internationalization messages
     * */
    function readMessages(){
        messages.submitSuccess = $('#cr_messages_submitSuccess').val();
        messages.ratingRequired = $('#cr_messages_ratingRequired').val();
    }
    ...
    $(function(){
        ...
        $("#editor_input").click( function(){clearServerCallFeedback(); });
        $("#chiefEditor_input").click(function(){ clearServerCallFeedback();});
    });

    return{
        readMessages : readMessages,
        readContentItem : readContentItem
    }
})();Bookmarks = {
	apiUrl : "",
	contentTitle : "",
...
var Tracker = {
	...
	trackP13nNotificationEvents : true,
	metricsArray : [],

	IMPRESSION_ITEM_RSB : "RSB_ITEM",//TopicSponsorship impressions, Add topic sponsorship (Related sponsor box or RSB)
	IMPRESSION_ITEM_MICROSITE : "MICROSITE_ITEM",//TopicPage Sponsorship impressions, Add sponsored topic page (Microsite)
	IMPRESSION_ITEM_RVC : "RVC_ITEM",
	IMPRESSION_BOX_RVC : "RVC_BOX",
	IMPRESSION_BOX_SPONSORED_1 : "SPONSORED_CONTENT_BOX_TYPE_1",
	IMPRESSION_BOX_SPONSORED_2 : "SPONSORED_CONTENT_BOX_TYPE_2",
	IMPRESSION_AD_DFP : "DFP_AD",
	...
	doTrackVcrImpressions : function() {
		...
		$(".f_sponsorship").each(
			function doTrack(index) {
				var $this = $(this);
				var pageType = InfoQConstants.pageType ? InfoQConstants.pageType : "PAGE_TYPE_UNAVAILABLE";

				var impressionJson = Tracker.getImpressionJsonForValues(Tracker.getJsonForSponsorshipItem($this),
					Tracker.IMPRESSION_ITEM_RSB, "impression",
					$this.data("place"), pageType, InfoQConstants.pageUrl, InfoQConstants.language, device );

				var viewableImpressionJson = Tracker.getImpressionJsonForValues(Tracker.getJsonForSponsorshipItem($this),
					Tracker.IMPRESSION_ITEM_RSB, "viewable",
					$this.data("place"), pageType, InfoQConstants.pageUrl, InfoQConstants.language, device );

				if(Tracker.isImpressionJsonValid(impressionJson)) {
					Tracker.metricsArray.push(impressionJson);
				}
				$.when(appearedInView($this, 0.5)).then(
					function(status) {
						if($this.is(":visible") && !$this.hasClass("hidden")) {
							if(Tracker.isImpressionJsonValid(viewableImpressionJson)) {
								Tracker.metricsArray.push(viewableImpressionJson);
								Tracker.checkIfMetricsCanBePosted();
							}
						}
					}
				);
		...
		$(".f_topicSponsorship").each(
			function doTrack(index) {
				var $this = $(this);
				var pageType = InfoQConstants.pageType ? InfoQConstants.pageType : "PAGE_TYPE_UNAVAILABLE";

				var impressionJson = Tracker.getImpressionJsonForValues(Tracker.getJsonForSponsorshipItem($this),
					Tracker.IMPRESSION_ITEM_MICROSITE, "impression",
					$this.data("place"), pageType, InfoQConstants.pageUrl, InfoQConstants.language, device );

				var viewableImpressionJson = Tracker.getImpressionJsonForValues(Tracker.getJsonForSponsorshipItem($this),
					Tracker.IMPRESSION_ITEM_MICROSITE, "viewable",
					$this.data("place"), pageType, InfoQConstants.pageUrl, InfoQConstants.language, device );
		...
		$(".f_rvcbox").each(function() {
			var pageType = InfoQConstants.pageType ? InfoQConstants.pageType : "PAGE_TYPE_UNAVAILABLE";
			var impressionJson = Tracker.getImpressionJsonForValues({}, Tracker.IMPRESSION_BOX_RVC, "impression",
				$(this).data("place"), pageType, InfoQConstants.pageUrl, InfoQConstants.language, device );
			var viewableImpressionJson = Tracker.getImpressionJsonForValues({}, Tracker.IMPRESSION_BOX_RVC, "viewable",
				$(this).data("place"), pageType, InfoQConstants.pageUrl, InfoQConstants.language, device );
		...
		$(".f_sponsorbox").each(function () {
			var element = $(this);
			var pageType = InfoQConstants.pageType ? InfoQConstants.pageType : "PAGE_TYPE_UNAVAILABLE";

			var impression_location = (pageType == "ARTICLE_PAGE" || pageType == "NEWS_PAGE") ? "RIGHTBAR_BOX_TYPE1_" : "";
			if(element.data("place") == "2" ||
				element.prevAll('.f_sponsorbox').length > 0 ||
				element.prevAll('.f_sponsorbox_top_1').length > 0 ||
				element.prevAll('.f_sponsorbox_top_2').length > 0 ||
				element.prevAll('.f_sponsorbox_bottom_1').length > 0 ||
				element.prevAll('.f_sponsorbox_bottom_2').length > 0) {
				impression_location += "BOTTOM";
			} else {
				impression_location += "TOP";
			}

			var interval = setInterval(function () {
				if(element.is(':visible')) {
					var impressionJson = Tracker.getImpressionJsonForValues({}, Tracker.IMPRESSION_BOX_SPONSORED_1, "impression",
						impression_location, pageType, InfoQConstants.pageUrl, InfoQConstants.language, device);
					if(Tracker.isImpressionJsonValid(impressionJson)) {
						Tracker.metricsArray.push(impressionJson);
					}
			...
			$.when(appearedInView(element, 0.5)).then(
				function (status) {
					if(element.is(':visible')) {
						var viewableImpressionJson = Tracker.getImpressionJsonForValues({}, Tracker.IMPRESSION_BOX_SPONSORED_1, "viewable",
							impression_location, pageType, InfoQConstants.pageUrl, InfoQConstants.language, device);
						if(Tracker.isImpressionJsonValid(viewableImpressionJson)) {
							Tracker.metricsArray.push(viewableImpressionJson);
							Tracker.checkIfMetricsCanBePosted();
		...
		$(".f_vcrdefault").each(
			function doTrack(index) {
				...
				var impressionJson = Tracker.getImpressionJsonForValues(Tracker.getJsonForDefaultVcrItem($this),
					Tracker.IMPRESSION_ITEM_RVC, "impression",
					$this.data("place"), pageType, InfoQConstants.pageUrl, InfoQConstants.language, device );
				var viewableImpressionJson = Tracker.getImpressionJsonForValues(Tracker.getJsonForDefaultVcrItem($this),
                    Tracker.IMPRESSION_ITEM_RVC, "viewable",
					$this.data("place"), pageType, InfoQConstants.pageUrl, InfoQConstants.language, device );
		...
		$(".f_vcroptional").each(
			function doTrack(index) {
				...
				var impression_place;
				if(element.data("place")) {
					if(element.data("place") == "1") {
						impression_place = "RIGHTBAR_BOX_TYPE1_TOP";
					} else if(element.data("place") == "2") {
						impression_place = "RIGHTBAR_BOX_TYPE1_BOTTOM";
					} else {
						impression_place = element.data("place");
					}
				} else {
					impression_place = "LOCATION_UNAVAILABLE";
				}


				var interval = setInterval(function () {
					if(element.closest('.vcr_optional').is(':visible') || device == "mobile") {
						var impressionJson = Tracker.getImpressionJsonForValues({"id":element.data("vcrid"), "title":element.data("vcrtitle")}, Tracker.IMPRESSION_ITEM_RVC, "impression",
							impression_place, pageType, InfoQConstants.pageUrl, InfoQConstants.language, device);
						if(Tracker.isImpressionJsonValid(impressionJson)) {
							Tracker.metricsArray.push(impressionJson);
						}
						clearInterval(interval);
				...
				$.when(appearedInView(device == "mobile"? element : element.closest('.row'), 0.5)).then(
					function(status) {
						if(element.closest('.vcr_optional').is(':visible') || device == "mobile") {
							var viewableImpressionJson = Tracker.getImpressionJsonForValues({"id":element.data("vcrid"), "title":element.data("vcrtitle")}, Tracker.IMPRESSION_ITEM_RVC, "viewable",
								impression_place, pageType, InfoQConstants.pageUrl, InfoQConstants.language, device);
							if(Tracker.isImpressionJsonValid(viewableImpressionJson)) {
								Tracker.metricsArray.push(viewableImpressionJson);
								Tracker.checkIfMetricsCanBePosted();
		...
		$.when(Tracker.isHiddenItemsInView()).then(function(){
			var pageType = InfoQConstants.pageType ? InfoQConstants.pageType : "PAGE_TYPE_UNAVAILABLE";
			$(".m-panel").find(".f_vcrdefault").each(function(){
				var $this = $(this);
				var viewableImpressionJson = Tracker.getImpressionJsonForValues(Tracker.getJsonForDefaultVcrItem($this),
                    Tracker.IMPRESSION_ITEM_RVC, "viewable",
					$this.data("place"), pageType, InfoQConstants.pageUrl, InfoQConstants.language, device );
			...
			Tracker.doPostMetric(JSON.stringify(Tracker.metricsArray));
			Tracker.metricsArray = [];
		});


		setTimeout(function () {
			$(".f_dfp_ad:visible").each(function(){
				var $this = $(this);
				var pageType = InfoQConstants.pageType ? InfoQConstants.pageType : "PAGE_TYPE_UNAVAILABLE";

				var impression_location = (pageType === "ARTICLE_PAGE" || pageType === "NEWS_PAGE") ? "RIGHTBAR_BOX_TYPE1_" : "";
				impression_location += $this.data("place").toUpperCase();
				if(impression_location === "TOP" || impression_location === "RIGHTBAR_BOX_TYPE1_TOP") {
					var prevAd = $this.prev(".f_dfp_ad");
					if(prevAd && prevAd.is(":visible")) {
						impression_location.replace("TOP", "BOTTOM");
					}
				}

				var impressionJson = Tracker.getImpressionJsonForValues({},
					Tracker.IMPRESSION_AD_DFP, "impression",
					impression_location, pageType, InfoQConstants.pageUrl, InfoQConstants.language, device );
				var viewableImpressionJson = Tracker.getImpressionJsonForValues({},
                    Tracker.IMPRESSION_AD_DFP, "viewable",
					impression_location, pageType, InfoQConstants.pageUrl, InfoQConstants.language, device );
		...
		}, ( window.noAdblocker === undefined ) ? 0 : 5000);
	...
	doTrackVcrRightbarImpressions : function(trackingClass) {
		...
		$("."+trackingClass).each(
			function doTrack(index) {
				...
				var vcrValues = trackingClass.split("_");
				var impression_location = "";
				if(vcrValues[4] && vcrValues[3]) {
					impression_location = "RIGHTBAR_BOX_TYPE" + vcrValues[4] + "_" + vcrValues[3].toUpperCase();
				} else {
					impression_location = "LOCATION_UNAVAILABLE";
				}

				var impressionJson = Tracker.getImpressionJsonForValues({"id":tvi, "title":$(this).text()}, Tracker.IMPRESSION_ITEM_RVC, "impression",
					impression_location, pageType, InfoQConstants.pageUrl, InfoQConstants.language, device );
				var viewableImpressionJson = Tracker.getImpressionJsonForValues({"id":tvi, "title":$(this).text()}, Tracker.IMPRESSION_ITEM_RVC, "viewable",
					impression_location, pageType, InfoQConstants.pageUrl, InfoQConstants.language, device );
	...
	doTrackVcrRightbarBoxesImpressions : function(trackingClass) {
		if(!Tracker.trackVcrRightbarImpressions) {
			return;
		}
		$("."+trackingClass).each(
			function doTrack(index) {
				...
				var item = "SPONSORED_CONTENT_BOX_TYPE_" + vcrValues[3];

				var impression_location = "";
				if(vcrValues[3] && vcrValues[2]) {
					impression_location = "RIGHTBAR_BOX_TYPE" + vcrValues[3] + "_" + vcrValues[2].toUpperCase();
				} else {
					impression_location = "LOCATION_UNAVAILABLE";
				}


				var impressionJson = Tracker.getImpressionJsonForValues({}, item, "impression",
					impression_location, pageType, InfoQConstants.pageUrl, InfoQConstants.language, device);
				var viewableImpressionJson = Tracker.getImpressionJsonForValues({}, item, "viewable",
					impression_location, pageType, InfoQConstants.pageUrl, InfoQConstants.language, device );
	...
	checkIfMetricsCanBePosted: function() {
		$.when(humanDetectionAsync()).then(
			function(status) {
				//console.log("Posting metrics after " + status);
				if(Tracker.metricsArray.length > 0) {
					Tracker.doPostMetric(JSON.stringify(Tracker.metricsArray));
					Tracker.metricsArray = [];
				}
			}
	...
	getImpressionJsonForValues: function(json, item, type, location, pageType, pageUrl, language, device) {
		json["type"] = "impression";
		json["pageUrl"] = pageUrl;
		json["language"] = language;
		json["device"] = device;
		json["impression_type"] = type;
		json["item"] = item;
	...
	getJsonForDefaultVcrItem: function(element) {
		if(element.hasClass("no-jsh")) {
			if (element.hasClass("no-data")) {
				return {"id": (element.find(".vcrid").text()?element.find(".vcrid").text():"ID_UNAVAILABLE"),
					"title": (element.find(".vcrtitle").text()?element.find(".vcrtitle").text():"TITLE_UNAVAILABLE")};
			} else {
				return {"id": (element.data("vcrid")?element.data("vcrid"):"ID_UNAVAILABLE"),
					"title": (element.data("vcrtitle")?element.data("vcrtitle"):"TITLE_UNAVAILABLE")};
			}
		} else {
			return JSON.parse(element.attr("jsh"));
		}
	},

	getJsonForSponsorshipItem: function (element) {
		if(element.attr("jsh") == undefined || element.attr("jsh") == "") {
			return "";
		}
		return JSON.parse(element.attr("jsh"));
	},

	isHiddenItemsInView: function() {
		var deferred = $.Deferred();
		$(".m-pop").hover(function() {
			$(".m-panel").find(".f_vcrdefault").each(function () {
				var $this = $(this);
				$this.viewable(function (event, percent) {
					if (percent >= 0.5) {
						deferred.resolve();
					}
				});
	...
	isImpressionJsonValid: function(json) {
        if(json["item"] == Tracker.IMPRESSION_ITEM_RVC && (json["id"] == "ID_UNAVAILABLE" || json["title"] == "TITLE_UNAVAILABLE")) {
            return false;
        }
		if(!json["pageType"] || $.inArray(json["pageType"], ["HOMEPAGE","NEWS_PAGE","ARTICLE_PAGE","INTERVIEW_PAGE","PRESENTATION_PAGE",
				"RESEARCH_PAGE","BOOK_PAGE", "PAGE_TYPE_UNAVAILABLE", "CATEGORY_PAGE","PODCAST_PAGE", "CONTENT_TYPE_PAGE", "CATEGORY_PAGE_BY_CONTENT_TYPE"]) == -1) {
			return false;
		}
		if(!json["location"] || ($.inArray(json["location"],["TOP","BOTTOM","EMBEDDED","RIGHTBAR_BOX_TYPE1_TOP",
				"RIGHTBAR_BOX_TYPE1_BOTTOM","RIGHTBAR_BOX_TYPE2_TOP","RIGHTBAR_BOX_TYPE2_BOTTOM", "LOCATION_UNAVAILABLE", "MICROSITE"]) == -1 &&
		   !json["location"].startsWith("EMBEDDED"))) {
			return false;
		}
		if(!json["item"] || $.inArray(json["item"],[Tracker.IMPRESSION_ITEM_RVC, Tracker.IMPRESSION_BOX_RVC, Tracker.IMPRESSION_BOX_SPONSORED_1,Tracker.IMPRESSION_BOX_SPONSORED_2, Tracker.IMPRESSION_AD_DFP, Tracker.IMPRESSION_ITEM_RSB, Tracker.IMPRESSION_ITEM_MICROSITE ]) == -1) {
			return false;
		}
		return true;
	...
	// P13N notifications metrics
	encodeNotificationLinks: function (container) {
		...
		$(container).find("a").each(function() {
			...
			if($(this).hasClass("f_notif_type_fulcnt")) {
				campaignType="followed_likes_content";
			}
			if($(this).hasClass("f_notif_type_fulcmt")) {
				campaignType="followed_likes_comment";
			}
			if($(this).hasClass("f_notif_type_ulmcnt")) {
				campaignType="user_likes_content";
			}
			if($(this).hasClass("f_notif_type_ulmcmt")) {
				campaignType="user_likes_comment";
			}
			if($(this).hasClass("f_notif_type_fupcnt")) {
				campaignType="followed_posts_content";
			}
			if($(this).hasClass("f_notif_type_fupcmt")) {
				campaignType="followed_posts_comment";
			}
			if($(this).hasClass("f_notif_type_fufu")) {
				campaignType="followed_followes_user";
			}
			if($(this).hasClass("f_notif_type_fuft")) {
				campaignType="followed_follows_topic";
			}
			if($(this).hasClass("f_notif_type_ufm")) {
				campaignType="user_followed_me";
			}
			if($(this).hasClass("f_notif_type_cntpft")) {
				campaignType="content_in_followed_topic";
			}

			var campaignCode = "utm_source=notification_web&utm_campaign=notifications&utm_medium=link";
			if(campaignType != "") {
				campaignCode += "&utm_content=" + campaignType;
				if(typeof loc != "undefined") {
					if(loc.indexOf('?')!=-1){
						$(this).attr("href", loc + "&" + campaignCode);
					}else{
						$(this).attr("href", loc + "?" + campaignCode);
					}
				}
	...
	doTrackP13nNotificationEvents : function(action, label) {
		if(!Tracker.trackP13nNotificationEvents) {
			return;
		}
		_gaq.push(['_trackEvent', 'notifications', action, label]);
	}
...
var InfoQABM = {
    record: function(status) {
        var abJson={};
        abJson["type"] = "ab_detect";
        abJson["status"] = status;
        abJson["pageUrl"] = InfoQConstants.pageUrl;
        abJson["pageType"] = InfoQConstants.pageType;
        abJson["language"] = InfoQConstants.language;
        abJson["device"] = device;
        abJson["loggedIn"] = loggedIn;
        Tracker.metricsArray.push(abJson);
        Tracker.checkIfMetricsCanBePosted();
    },
    ...
    process: function() {
        ...
        element.onerror = function () {
            if(CookieManager.readCookie('infoq_hab') !== 'ooodnd') {
                CookieManager.createCookie('infoq_hab', 'oooy');
                //adblocker on
                InfoQABM.record("y");
                InfoQABM.showMessage();
            } else {
                //adblocker on, dnd
                InfoQABM.record("ydnd");
            }
        };
        element.onload = function () {
            if (InfoQABM.isAdBlocked()) {
                if(CookieManager.readCookie('infoq_hab') !== 'ooodnd') {
                    CookieManager.createCookie('infoq_hab', 'oooy');
                    //adblocker on
                    InfoQABM.record("y");
                ...
                if (CookieManager.readCookie('infoq_hab') === 'oooy') {
                    CookieManager.createCookie('infoq_hab', 'ooor');
                    //just whitelisted
                    InfoQABM.record("r");
                } else {
                    if (CookieManager.readCookie('infoq_hab') === 'ooor') {
                        //adblocker off by whitelisting after showing the message
                        InfoQABM.record("nr");
                    }
                    else {
                        if(CookieManager.readCookie('infoq_hab') === 'ooodnd') {
                            //adblocker off, disabled by user by his own will but we showed him the message at some point before
                            InfoQABM.record("ndnd");
                        } else {
                            //adblocker off since the beginning
                            InfoQABM.record("n");
                        }
                    }
                }
    ...
    showMessage: function() {
        //disable the message for mobile devices
        //if(device === "mobile") return;
        //disable the message asking user to whitelist infoq.com, keeping statistics
        if(1==1) return;
        ...
        $(document).ready(function () {
            $(document).on("scroll", function () {
                ...
                if (y > 400) {
                    $('.adblockermessage').fadeIn('slow');
                    $(document).off('scroll');
                    InfoQABM.record("i");
                }
            });

            $('.close-adblockermessage').click(function () {
                CookieManager.createCookie('infoq_hab', 'ooodnd');
                $('.adblockermessage').hide();
            });
        });
    },
...
/*	$("a.reply").click(function() {
	...
	$(".logged_in").click(function() {
		$("#login").toggleClass('over');
		// hide notification widget if displayed
		$('.notificationWidget__dropdown').css('display','none');
		return false;
	});   
...
//		}
//	});
});var humanDetectionAsync = function() {
    var deferred = $.Deferred();

    var $window = $(window);
    $window.mousemove(function() {
        deferred.resolve("mousemove");
    });
    $window.scroll(function() {
        deferred.resolve("scroll");
    });
    $window.click(function(){
        deferred.resolve("click");
    });
    $window.keydown(function() {
        deferred.resolve("keydown");
    });
    $window.on("touchstart", function() {
        deferred.resolve("touchstart");
    });
...
function checkVisibility() {
	...
	$.each(elements, function(i, element) {
		...
		setTimeout(function() {
			// Get element top offset and height
			var elTop2      = $(element).offset().top,
				elHeight2  = parseInt( $(element).css('height') ),
				elBottom2   = elTop2 + elHeight2,
				percentage2 = 0, hiddenTop2  = 0, hiddenBottom2 = 0;

			// Get percentage of unviewable area
			if ( elTop2 < winTop )             // Area above the viewport
				hiddenTop2 = winTop-elTop2;
			if ( elBottom2 > winBottom )       // Area below the viewport
				hiddenBottom2 = elBottom2-winBottom;

			percentage2 = 1 - ((hiddenTop2 + hiddenBottom2)/elHeight2);
			if(percentage2 >= percentage) {
				$(element).trigger('viewable', [ (percentage > 1 ? 1 : percentage < 0 ? 0 : parseFloat(percentage)) ]);
			}
			//in view for at least one second
		}, 900);
...
})(jQuery);var processing = false;
var UserActions_Personalization = {
    followTopic: function (isFollowing, id, btnId) {
        UserActions_Personalization.requestFollow(isFollowing, id, btnId);
    },
    likeContent: function (isLiked, id, path, title, authorUserCSVIds) {
        UserActions_Personalization.requestLikeContent(isLiked, id, path, title, authorUserCSVIds);
        if(!isLiked) {
            _gaq.push(['_trackEvent', 'like', 'like_content', 'like_content']);
        } else {
            _gaq.push(['_trackEvent', 'like', 'unlike_content', 'unlike_content']);
        }
    },
    likeComment: function (id, threadId, subject, userId, path, title) {
        UserActions_Personalization.requestLikeComment(id, threadId, subject, userId, path, title);
        _gaq.push(['_trackEvent', 'like', 'like_comment', 'like_comment']);
    },
    followUser: function (isFollowing, id, btnId) {
        UserActions_Personalization.requestFollow(isFollowing, id, btnId);
        if(!isFollowing) {
            _gaq.push(['_trackEvent', 'user_page', 'follow_button_click', 'follow_button_click']);
        } else {
            _gaq.push(['_trackEvent', 'user_page', 'unfollow_button_click', 'unfollow_button_click']);
        }
    },
    followSmall: function (followVarName, id, target, f, group) {
        UserActions_Personalization.requestFollowSmall(followVarName, id, '#'+target+f+id, target, group);
    },
    requestFollow: function (isFollowing, id, btnId) {
        if (!processing) {
            actionName = btnLabelBig === '' ? 'User' : 'Topic';
            processing = true;

            $(btnId).prop('disabled', true);
            UserActions_Personalization.setupFollowBig(true, btnId);
            $.ajax({
                type: "GET",
                url: InfoQConstants.countryCode + "/fandl!follow"+actionName+".action",
                data: {"id": id, "f": !isFollowing},
                success: function (data) {
                    if (data === null || data.msg !== "OK") {
                        UserActions_Personalization.setupFollowBig(true, btnId);
                        UserActions_Personalization.errorMessage();
                    }
                    //UserActions_Personalization.setupFollowBig(true, labelUserOrTopic);
                    UserActions_Personalization.setupFollowButtonSmallAll(true, 'isFollowedTopic' + id, actionName + id, true);
    ...
    requestFollowSmall: function (followVarName, id, btnId, target, group) {
        if (!processing) {
            processing = true;

            if(typeof group !== "undefined") {
                btnId=btnId.substring(1);//remove hash (TODO unify)
            } else {
                group = ""; //it is used for method name string concat
            }
            $(btnId).prop('disabled', true);
            UserActions_Personalization["setupFollowButtonSmall"+group](true, followVarName, btnId);

            $.ajax({
                type: "GET",
                url: InfoQConstants.countryCode + "/fandl!follow"+target+".action",
                data: {"id": id, "f": window[followVarName]},
                success: function (data) {
                    if (data === null || data.msg !== "OK") {
                        UserActions_Personalization["setupFollowButtonSmall"+group](true, followVarName, btnId);
                        UserActions_Personalization.errorMessage();
                    }
                    //UserActions_Personalization.setupFollowButtonSmall(true, followVarName, btnId);
                    $(btnId).prop('disabled', false);
                    processing = false;
                },
                error: function () {
    ...
    requestLikeContent: function (isLiked, id, path, title, authorUserCSVIds) {
        if (!processing) {
            processing = true;
            $('#toggleLikeContent').prop('disabled', true);
            UserActions_Personalization.setupLikeContentButton(true);
            $.ajax({
                type: "POST",
                url: InfoQConstants.countryCode + "/fandl!likeContent.action",
                data: {"id": id, "path":path, "title":title ,"f": !isLiked, "aids":authorUserCSVIds},
                success: function (data) {
                    if (data === null || data.msg !== "OK") {
                        UserActions_Personalization.setupLikeContentButton(true);
                        UserActions_Personalization.errorMessage();
                    ...
                    //UserActions_Personalization.setupLikeContentButton(true);
                    $('#toggleLikeContent').prop('disabled', false);
                    processing = false;
                },
                error: function () {
                    UserActions_Personalization.setupLikeContentButton(true);
    ...
    requestLikeContentListing: function(isLiked, contentId, path, title, authors) {
        if (!processing) {
            processing = true;
            $.ajax({
                type: "POST",
                url: InfoQConstants.countryCode + "/fandl!likeContent.action",
                data: {"id": contentId, "path":path, "title":title ,"f": !isLiked, "aids":authors},
                success: function (data) {
                    if (data === null || data.msg !== "OK") {
                        UserActions_Personalization.errorMessage();
                    }
                    UserActions_Personalization.setupLikeContentListing(true, contentId);
                    processing = false;
                },
                error: function () {
    ...
    requestLikeComment: function (id, threadId, subject, userId, path, title) {
        if (!processing) {
            processing = true;
            var btnId = 'toggleLikeComment' + id;
            $('#' + btnId).prop('disabled', true);
            var isLiked = isCommentLiked(id);
            UserActions_Personalization.setupLikeCommentButton(true, id);
            $.ajax({
                type: "POST",
                url: InfoQConstants.countryCode + "/fandl!likeComment.action",
                data: {"id": threadId + '-' + id, "f": !isLiked,"usid":userId,"contentPath":path, "contentTitle":title, "commentSubject":subject},
                success: function (data) {
                    if (data === null || data.msg !== "OK") {
                        UserActions_Personalization.setupLikeCommentButton(true, id);
                        UserActions_Personalization.errorMessage();
    ...
    errorMessage: function () {
        alert("ERROR: Please contact support! Thank you.");
    },
    setupFollowBig: function (toggle, btnId) { /*TODO P13N the "follow" functions can be refactored to one*/
    if(typeof btnForceHide === "undefined" || !btnForceHide) {
        $(btnId).css('display', showFollowAndLike);
        if (loggedIn) {
            if (toggle) {
            ...
            $(btnId).removeClass();
            if (isFollowed) {
                $(btnId).addClass('user-button-grey').html('<span class="icon-remove_circle_outline"></span>' + followBtnLabels[1] + ' ' + btnLabelBig);
            } else {
                $(btnId).addClass('user-button-blue').html('<span class="icon-add_circle_outline"></span>' + followBtnLabels[0] + ' ' + btnLabelBig);
            }
        }
    }
    },
    setupFollowButtonSmall: function (toggle, followVarName, btnId) {
        if (toggle) {
            window[followVarName]=!window[followVarName];
            $(btnId).removeClass();
            $(btnId).addClass('user-button-small');
        }
        /*if close button is displayed toggle it*/
        hideBtn = $(btnId).parent().parent().find('.hide-item');
        if (hideBtn.length && hideBtn.css('display') !== 'none') {
            hideBtn.css('visibility', window[followVarName]?'visible':'hidden');
        }
        
        if (window[followVarName]) {
            $(btnId).addClass('user-button-grey').html('<span class="icon-remove_circle_outline"></span>'+followBtnLabels[1]);
        } else {
            $(btnId).addClass('user-button-blue').html('<span class="icon-add_circle_outline"></span>'+followBtnLabels[0]);
        }
    },
    setupLikeContentButton: function (toggle) {
        $("#spanLikeContent").css('display', showFollowAndLike);
        if (toggle) {
            isLikedContent = !isLikedContent;
            $('#toggleLikeContent').off('mouseenter mouseleave');
            $('#toggleLikeContent').removeClass();
        }
        if (isLikedContent) {
            $('#toggleLikeContent').addClass('button--like');
            $('#toggleLikeContent').addClass('button--liked').html('<span class="button--like__icon"></span>'+likeBtnLabels[1]);
            $('#toggleLikeContent').css('color', 'white');
        } else {
            $('#toggleLikeContent').addClass('button--like').html('<span class="button--like__icon"></span>'+likeBtnLabels[0]);
            $('#toggleLikeContent').css('color', '#286ab2');
        }
    },
    setupLikeCommentButton: function (toggle, id) {
        $("#spanLikeComment" + id).css('display', showFollowAndLike);
        var btnId = 'toggleLikeComment' + id;
        var commentLiked = isCommentLiked(id);
        if (toggle) {
            commentLiked = !commentLiked;
            $('#' + btnId).removeClass();
        }
        if (commentLiked) {
            $('#' + btnId).addClass('button--comment').html(likeBtnLabels[1]);
            addCommentId(id);
        } else {
            $('#' + btnId).addClass('button--comment').html(likeBtnLabels[0]);
            removeCommentId(id);
        }
    },
    requestCounts: function (userToken) {
        if (!processing) {
            processing = true;
            $.ajax({
                type: "GET",
                url: InfoQConstants.countryCode + "/userProfile!getCounts.action",
                ...
                success: function (data) {
                    ...
                    Object.keys(data).forEach(function(k) {
                        if(k !== "msg") {
                            if(data[k] === 0) {
                                $('#'+k+"TotalCount").parent().parent().css('display', 'none');
                            }
                            $('#'+k+"TotalCount").text(data[k]);
                        }
                    });
                    processing = false;
                },
    ...
    setupFollowButtonSmallAll: function (toggle, followVarName, btnId, requestFromBigButton) {
        if (toggle) {
            window[followVarName]=!window[followVarName];
        }
        $('a[id="' + btnId + '"]').each(function (idx, val) {
            $(val).removeClass('user-button-blue');
            UserActions_Personalization.setupFollowButtonSmallInGroup(toggle, followVarName, val);
        });
        if (toggle && typeof requestFromBigButton === "undefined") { /*check big buttons only if requested*/
            $('a[id="Big' + btnId + '"]').each(function (idx, val) {
                UserActions_Personalization.setupFollowBig(toggle, val);
            });
        }
    },
    setupLikeContentListing: function(toggle, contentId) {
        if (loggedIn) {
            if(toggle) {
                window["isLikedContent" + contentId] = !window["isLikedContent" + contentId];
                if (window["isLikedContent" + contentId]) {
                    $('a[id="Content' + contentId + '"]').removeClass('like-link--grey');
                } else {
                    $('a[id="Content' + contentId + '"]').removeClass('like-link--on');
                }
            }
            if (window["isLikedContent" + contentId]) {
                $('a[id="Content' + contentId + '"]').addClass('like-link--on').html('Liked');
            } else {
                $('a[id="Content' + contentId + '"]').addClass('like-link--grey').html('Like');
            }
        }
    },
    setupFollowButtonSmallInGroup: function (toggle, followVarName, btnId) {
        if (toggle) {
            $(btnId).removeClass();
            $(btnId).addClass('user-button-small');
        }
    ...
    checkFollowList: function (theMap) {
        $.ajax({
            type: "GET",
            url: InfoQConstants.countryCode + "/fandl!checkGroupFollowList.action",
            data: {"id": JSON.stringify(theMap)},
            success: function (data) {
                if (data === null || data.msg !== "OK") {
                    UserActions_Personalization.errorMessage();
                } else {
                    if (data.followedTopicIdList.length > 0) {
                        $.each(JSON.parse(data.followedTopicIdList), function (index, value) {
                            var tvalue = "Topic" + value;
                            window['isFollowed' + tvalue] = true;
                            UserActions_Personalization.setupFollowButtonSmallAll(false, 'isFollowed' + tvalue, tvalue);
                        });
                    }
                    if (data.followedUserIdList.length > 0) {
                        $.each(JSON.parse(data.followedUserIdList), function (index, value) {
                            var tvalue = "User" + value;
                            window['isFollowed' + tvalue] = true;
                            UserActions_Personalization.setupFollowButtonSmallAll(false, 'isFollowed' + tvalue, tvalue);
                        });
                    }
                    if (data.likedContentIdList.length > 0) {
                        var likedContentIds = data.likedContentIdList.replace(/\'/g, '"')
                        $.each(JSON.parse(likedContentIds), function (index, value) {
                            window['isLikedContent' + value] = true;
                            UserActions_Personalization.setupLikeContentListing(false, value);
                        });
                    }
                }
            },
    ...
    prepareGroupFollowData: function () {
        theMap = {
            "Topic": ptCatIdList,
            "User": authorIdList,
            "Content": contentIdList
        };
        if(theMap.Topic.length > 0 || theMap.User.length > 0) {
            if(loggedIn) {
                UserActions_Personalization.checkFollowList(theMap);
            }
        }
        if(theMap.Topic.length > 0) {
            UserActions_Personalization.setupGroupFollowButtons(theMap.Topic, "Topic");
        }
        if(theMap.User.length > 0) {
            UserActions_Personalization.setupGroupFollowButtons(theMap.User, "User");
        }
        if (theMap.Content.length > 0) {
            UserActions_Personalization.setupListingsLikeButtons(theMap.Content, "Content");
        }
    },
    setupGroupFollowButtons: function (idList, target) {
        if (idList.length > 0) {
            $.each(idList, function (idx, value) {
                var tvalue = target + value;
                $('a[id="' + tvalue + '"]').each(function (i, v) {
                    if (!loggedIn) {
                        $(v).addClass('authModal--trigger');
                        $(v).on('click', function () {
                            $('#p13n-id').val(value);
                            $('#p13n-type').val('follow' + target);
                        });
                        window['isFollowed' + tvalue] = false;
                    } else {
                        $(this).click(function () {
                            UserActions_Personalization.followSmall('isFollowed' + tvalue, value, target, '', 'All');
                            return false;
                        });
                    }
                });
    ...
    setupListingsLikeButtons: function(idList) {
        if (idList.length > 0) {
            $.each(idList, function (idx, value) {
                $('a[id="Content' + value + '"]').each(function (i, v) {
                    if (!loggedIn) {
                        $(v).addClass('authModal--trigger');
                        $(v).on('click', function () {
                            $('#p13n-id').val(value);
                            $('#p13n-type').val('content');
                        });
                        window['isLikedContent' + value] = false;
                    } else {
                        var pathSelector = 'Content' + value + '_path';
                        var contentPath = $('input#' + pathSelector).val();

                        var titleSelector = 'Content' + value + '_title';
                        var contentTitle = $('input#' + titleSelector).val();

                        var authorsSelector = 'Content' + value + '_authorIds';
                        var contentAuthors = $('input#' + authorsSelector).val();

                        $(this).click(function () {
                            var isLiked = window['isLikedContent' + value];
                            UserActions_Personalization.requestLikeContentListing(isLiked, value, contentPath, contentTitle, contentAuthors);
                            return false;
                        });
                    }
                });
...
function backBtnCheck(idValue) {
    var hiddenInputId='#reloadValue_'+idValue;
    ...
    if (ua.indexOf('chrome') > -1) {
        ...
        if (jQuery(hiddenInputId).val().length === 0) {
            jQuery(hiddenInputId).val(d);
            jQuery('body').show();
        } else {
...
var TopBarPromoWidget = {
    init : function() {
        if (CookieManager.readCookie('topbarSurvey') !== 'hide') {
            $(document).ready(function () {
                $('.top-bar-promo').slideDown();
                $('.top-bar-close').click(function () {
                    $('.top-bar-promo').slideUp();
                    CookieManager.createDayCookie('topbarSurvey', 'hide', 3650);
                });
            });
        }
    }
